...
ggplot() + geom_density(data = abia , aes(x= ArrDelay,fill="Arrival Delay"),alpha=0.3 , size = 0.25)+ geom_density(data = abia , aes(x= DepDelay,fill="Departure Delay") ,alpha=0.3, size = 0.25)+facet_wrap(facets = vars(UniqueCarrier),scales = 'free') + xlim(c(-50,100)) +theme_custom() + scale_fill_manual(name='Delay Type\n', values = c('blue','salmon'))
...
ggplot() + geom_point(data = abia_arr , aes(x= ArrTime, y= ArrDelay , color = Distance),alpha=0.3 , size = 2.25)+facet_wrap(facets = vars(UniqueCarrier),scales = 'free_y') +theme_custom()
...
ggplot() + geom_point(data = abia_dep , aes(x= DepTime, y= DepDelay , color = Distance),alpha=0.3 , size = 2.25)+facet_wrap(facets = vars(UniqueCarrier),scales = 'free_y') +theme_custom()

```

We avail the insight into Departure delay for flights flying out from Austin. Times with multiple flights lined one after another are 



## 3 - Portfolio Modeling

**Portfolio includes 6 ETF's which has a mix of large cap blend equities, mid cap blend equities, small cap blend equities, oil/gas commodity ETF and alternative energy equities. Blend equity ETF's are a mixture of growth as well as value equities.**

**For the first possibility, 40% capital is allocated for large cap ETF, 10% capital to small and mid cap ETF's each, 20% each for alternative energy and oil/gas commodities. For this allocation, if we invest $100k, we are 95% confident that our worst 20 trading day loss will not exceed $8052.51 with an average profit of $1683.2**
...
all_returns_energy_cap = as.matrix(na.omit(all_returns_energy_cap))

# Compute the returns from the closing prices
pairs(all_returns_energy_cap)
```
